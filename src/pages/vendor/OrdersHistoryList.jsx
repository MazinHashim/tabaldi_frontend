    ...
    return [
        ...
        ...filteredOrders.map(order => {
            ...
            return [
                ...
                order.vendor?.region || '',
                order?.forderDate || '',
                `${customerPrice.toFixed(2)} ${t("aedUnit")}`,
                `${vendorPrice.toFixed(2)} ${t("aedUnit")}`,
                `${rateenaProfit.toFixed(2)} ${t("aedUnit")}`,
                `${order.cartItems?.[0]?.product?.companyProfit || 0}%`,
                `${order?.shippingCost || 0} ${t("aedUnit")}`,
                order?.paymentMethod || '',
                order?.status || '',
                order?.orderNumber || '',
            ];
            ...
            <div className='flex flex-col w-full'>
                <div className="flex flex-col shadow-4 p-2 rounded-2xl">
                    <div className="flex justify-between flex-wrap">
                        ...
                        <button onClick={exportToExcel} className='bg-primary-color text-white p-2 m-2 rounded-lg'>Export to Excel</button>
                    </div>
